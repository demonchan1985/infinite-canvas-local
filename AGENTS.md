...
- 本目录为用户批准的 TOPAPIS 风格独立版。仅在本目录修改；禁止回写、同步或覆盖 `basketikun-infinite-canvas` 原目录。
- 不配置上游自动更新。前端固定 3102，Agent 固定 17376，Agent 数据仅写入本目录 `canvas-agent/.runtime/`。
- 本次用户已批准参考站的悬浮圆角工具栏，优先于下文原版的“顶部无胶囊背景”规范；保留现有节点数据和渲染引擎。
