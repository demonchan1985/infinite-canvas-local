...
+ [调整] 图片反推提示词的专用配置卡片不再弹出或提供“组装提示词”创作框，保留节点连线、模型设置与卡片内生成操作。
+ [修复] 图片节点“反推提示词”使用本机 Codex CLI 文本模型时，已连接的参考图随请求传入 CLI，不再只发送文字导致模型要求重新上传图片；图片不可读取时明确报错。
+ [修复] 本机 Codex CLI 渠道的验证连接与模型摘取直接调用 Codex app-server 的模型列表，不再依赖旧 17376 接口或 API Key；连接失败时显示具体原因。
+ [优化] 图片节点悬停工具条中的存资产图标在简洁模式下也直接显示，鼠标提示明确为“添加到资产库”；继续复用原有图片资产保存与成功提示。
+ [调整] 正式主页采用 A1 节点舞台样式与三幕创作场景轮播，背景跟随最近画布色卡及日夜皮肤；主页暂不显示 Logo。
+ [调整] 项目内 Codex CLI 升级至 0.156.1，使独立 Agent 识别当前账号的 GPT-6 编排模型并保持本机 GPT Image 2.5 生图可用。
+ [修复] 本机 Codex 的 GPT Image 2.5 图片请求改用当前账号可用的默认编排模型，并按 Codex Responses 协议启用流式、关闭存储，避免固定旧模型导致生图失败。
