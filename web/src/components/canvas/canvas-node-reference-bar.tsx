...
export function CanvasNodeReferenceBar({ nodeId, nodes, connectedNodes, onDisconnect, onStartSelection, showAddButton = true }: { nodeId: string; nodes: CanvasNodeData[]; connectedNodes: CanvasNodeData[]; onDisconnect?: (fromNodeId: string, toNodeId: string) => void; onStartSelection?: (nodeId: string) => void; showAddButton?: boolean }) {
    ...
    if (!references.length && !showAddButton) return null;
    return (
        <div className="mb-2 flex min-w-0 items-center gap-2">
            <div className="thin-scrollbar flex min-w-0 flex-1 gap-2 overflow-x-auto py-0.5">
            ...
            {showAddButton ? <CanvasNodeReferenceAddButton nodeId={nodeId} onStartSelection={onStartSelection} /> : null}
        </div>
    );
}

export function CanvasNodeReferenceAddButton({ nodeId, onStartSelection }: { nodeId: string; onStartSelection?: (nodeId: string) => void }) {
    const { t } = useTranslation();
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    return (
        <button type="button" className="grid size-12 shrink-0 place-items-center rounded-xl border bg-transparent transition hover:opacity-70" style={{ borderColor: theme.toolbar.border, color: theme.node.muted }} title={t("canvas.references.select")} aria-label={t("canvas.references.select")} onClick={() => onStartSelection?.(nodeId)}>
